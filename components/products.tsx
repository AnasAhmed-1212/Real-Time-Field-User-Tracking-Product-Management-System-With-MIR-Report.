...
export function Products() {
  const { data, setData, loading, error, refresh } = useAdminResource<Product[]>("products", [])
  ...
  async function create(event: FormEvent) {
    ...
    try {
      ...
      const created = await adminApi<Product>("products", { method: "POST", body: JSON.stringify({ ...fields, price: Number(fields.price), image: imageUrl ? { url: imageUrl } : undefined }) })
  ...
  return <main className="flex-1 bg-muted/20 p-4 sm:p-6"><div className="mx-auto max-w-7xl space-y-5">
    ...
    {actionError && <ApiError title="Unable to update product" message={actionError} />}
